...
func main() {
	var err error
	psqlInfo := fmt.Sprintf("host=%s port=%d user=%s"+"password=%s dbname=%s sslmode=disabled", host, port, user, password, db)
...
func Insert(ctx context.Context) {
	sqlStatement := `INSERT INTO users (age, email, first_name,last_name)
					 VALUES ($1, $2, $3, $4)`
	// no data returned from the query than use ExecContext()
